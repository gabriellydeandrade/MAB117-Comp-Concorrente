...
void testa_se_ordena_corretamente_independente_da_quantidade_de_threads(){
    ...
    for (qtd_threads=1; qtd_threads<tamanho_vetor; qtd_threads++){
        int *resultado = merge_sort(tamanho_vetor, vetor, qtd_threads);

        if (vetores_iguais(resultado, vetor_esperado))
            printf(ANSI_COR_SUCESSO "Teste `testa_se_ordena_corretamente_independente_da_quantidade_de_threads` passou \n" ANSI_COR_PADRAO);
        else{
            printf(ANSI_COR_FALHA "Teste `testa_se_ordena_corretamente_independente_da_quantidade_de_threads` falhou \n" ANSI_COR_PADRAO);
