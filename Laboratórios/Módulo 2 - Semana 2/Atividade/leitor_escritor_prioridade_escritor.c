...
#define NTHREADS_ESCRITORA 4 //numero de threads escritoras
#define TAM_VETOR 5
...
void inicio_leitura(int id) {
    ...
    printf("L[%d] quer ler\n", id);
    while(tamanho_fila_escrita > 0 || qtd_tarefas_escrevendo > 0) {
        printf("L[%d] bloqueou\n", id);
...
void fim_escrita(int id) {
    ...
    if (tamanho_fila_escrita == 0) pthread_cond_broadcast(&condicao_leitura);
    pthread_mutex_unlock(&mutex);
}
